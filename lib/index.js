...
function doctor() {
  let output;
  try {
    output = (0, import_child_process.execSync)("serverpod version");
  } catch (e) {
  ...
  import_coc.window.showInformationMessage("Serverpod CLI is installed and up to date.");
}
function validVersion(versionString) {
  console.log(versionString);
  const versionTag = versionString.split(":")[1];
  const versionNumber = (0, import_semver.coerce)(versionTag);
  if (versionNumber === null) {
    return true;
  }
  return (0, import_semver.satisfies)(versionNumber, ">=2.2.0");
}
